...
window.startAutoTrading = async function startAutoTrading(){
...
window.stopAutoTrading = async function stopAutoTrading(){
...
/* ============================================================
   🦁 LION AI PRO — AUTO PAPER BUTTON FIX V2
   ============================================================ */
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    console.log("🦁 AUTO BUTTON FIX V2 LOADED");
    ...
    if (startBtn) {
        startBtn.addEventListener("click", async function () {
            ...
            try {
                ...
                console.log("🦁 PAPER START RESPONSE:", data);

                if (!response.ok || !data.ok) {
                    throw new Error(
                        data.error || data.message || "Paper Trading error"
                    );
                }
                ...
                if (typeof loadPaperStatus === "function") {
                    setTimeout(loadPaperStatus, 300);
                }
    ...
    if (stopBtn) {
        stopBtn.addEventListener("click", async function () {
            ...
            if (status)
                status.textContent = "● در حال توقف...";

            try {

                const response = await fetch(
                    API + "/paper/stop?t=" + Date.now(),
                    {
                        method: "POST",
                        cache: "no-store"
                    }
                ...
                console.log("🦁 PAPER STOP RESPONSE:", data);
